# SecureEnvelopeKit (Swift)

Product-independent secure envelope protocol kit — the Apple (iOS/macOS)
implementation. This is one platform of the
[SecureEnvelopeKit monorepo](../../README.md); the language-neutral contract and
shared fixture live at the repository root under `docs/spec/` and `fixtures/`.
...
The stable v1 fixture lives at the repository root in
`fixtures/SecureEnvelopeV1/secure-envelope-v1.json` (shared by every platform).
The Swift tests read it directly from there. The Android and .NET
implementations use the same fixture for their parity tests, and no platform
integrates with an app until it can open the fixture envelope and reproduce the
same deterministic encoding. The language-neutral contract is in
`docs/spec/secure-envelope-v1.md` at the repository root.
...
The v1 binary format is shared with the Android/Kotlin (`platforms/android`) and
.NET (`platforms/dotnet`) implementations in this repository through the common
spec and fixture, so envelopes interoperate across platforms.
